...
static void update_timeout_handler(void * p_context)
{
    ...
    if (m_conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        {
            // Check the current connection parameters
            if (!is_conn_params_ok(&m_current_conn_params))
            {
                uint32_t err_code;
                
                // Parameters are not ok, send connection parameters update request.
                err_code = ble_gap_conn_param_update(m_conn_handle, &m_preferred_conn_params);
        ...
        else
        {
            ...
            if (m_conn_params_config.disconnect_on_fail)
            {
                ...
                err_code = ble_gap_disconnect(m_conn_handle, BLE_HCI_CONN_INTERVAL_UNACCEPTABLE);
                if ((err_code != NRF_SUCCESS) && (m_conn_params_config.error_handler != NULL))
                {
                    m_conn_params_config.error_handler(err_code);
                }
...
uint32_t ble_conn_params_init(const ble_conn_params_init_t * p_init)
{
    ...
    if (p_init->p_conn_params != NULL)
    {
        ...
        err_code = ble_gap_ppcp_set(&m_preferred_conn_params);
    ...
    else
    {
        ...
        err_code = ble_gap_ppcp_get(&m_preferred_conn_params);
        if (err_code != NRF_SUCCESS)
        {
            return err_code;
        }
