...
/**@brief Set GLS communication state.
 *
...
/**@brief Set next sequence number by reading the last record in the data base.
 *
...
/**@brief Add characteristic for glucose measurement.
 *
...
static uint32_t glucose_measurement_char_add(ble_gls_t * p_gls)
{
    ...
    return ble_gatts_characteristic_add(p_gls->service_handle,
                                        &char_md,
                                        &attr_char_value,
                                        &p_gls->glm_handles);
}
...
/**@brief Add characteristic for glucose feature.
 *
 * @param[in]   p_gls   Service instance.
 *
 * @return      NRF_SUCCESS if characteristic was successfully added, otherwise an error code.
...
/**@brief Add characteristic for record access control point.
 *
 * @param[in]   p_gls   Service instance.
 *
 * @return      NRF_SUCCESS if characteristic was successfully added, otherwise an error code.
...
static uint32_t record_access_control_point_char_add(ble_gls_t * p_gls)
{
    ...
    char_md.char_properties.indicate = 1;
...
uint32_t ble_gls_init(ble_gls_t * p_gls, const ble_gls_init_t * p_gls_init)
{
    ...
    err_code = ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &p_gls->service_handle);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
/**@brief Send response from Record Access Control Point.
 *
 * @param[in]   p_gls        Service instance.
...
static void racp_send(ble_gls_t * p_gls, ble_racp_value_t * p_racp_val)
{
    ...
    err_code = ble_gatts_hvx(p_gls->conn_handle, &hvx_params);
...
/**@brief Send a RACP response containing a Response Code Op Code and Response Code Value.
 *
 * @param[in]   p_gls    Service instance.
...
/**@brief Respond to ALL operation.
 *
 * @param[in]   p_gls   Service instance.
 *
 * @return      NRF_SUCCESS on success, otherwise an error code.
...
/**@brief Respond to FIRST or LAST operation.
 *
 * @param[in]   p_gls   Service instance.
 *
 * @return      NRF_SUCCESS on success, otherwise an error code.
...
/**@brief Respond to GREATER_OR_EQUAL operation.
 *
 * @param[in]   p_gls   Service instance.
 *
 * @return      NRF_SUCCESS on success, otherwise an error code.
...
/**@brief Inform that REPORT RECORDS procedure is completed.
 *
 * @param[in]   p_gls   Service instance.
 */
...
/**@brief RACP report records procedure.
 *
 * @param[in]   p_gls   Service instance.
 */
...
/**@brief Test if the received request is to be executed.
 *
                                      ...
                                      uint8_t *                p_response_code)
{
    *p_response_code = RACP_RESPONSE_RESERVED;
    ...
    {
        // TODO: According to the spec, we should send a RACP response in this situation
        return false;
    }
    // supported opcodes
...
/**@brief Process a REPORT RECORDS request.
 *
 * @param[in]   p_gls            Service instance.
...
/**@brief Process a REPORT NUM RECORDS request.
 *
 * @param[in]   p_gls            Service instance.
 * @param[in]   p_racp_request   Request to be executed.
 */
...
/**@brief check if the cccds are configured 
 *
 * @param[in]   p_gls                   Service instance.
...
uint32_t ble_gls_are_cccd_configured(ble_gls_t * p_gls, bool * p_are_cccd_configured)
{
    ...
    err_code = ble_gatts_value_get(p_gls->glm_handles.cccd_handle,
                                   0,
                                   &len,
                                   cccd_value_buf);
    if (err_code != NRF_SUCCESS)
    ...
    err_code = ble_gatts_value_get(p_gls->racp_handles.cccd_handle,
                                   0,
                                   &len,
                                   cccd_value_buf);
    if (err_code != NRF_SUCCESS)
...
/**@brief Handle a write event to the Record Access Control Point.
 *
 * @param[in]   p_gls         Service instance.
...
static void on_racp_value_write(ble_gls_t * p_gls, ble_gatts_evt_write_t * p_evt_write)
{
    ...
    if (!are_cccd_configured)
    {
        ...
        err_code = ble_gatts_rw_authorize_reply(p_gls->conn_handle, &auth_reply);
        if (err_code != NRF_SUCCESS)
        {
            if (p_gls->error_handler != NULL)
            {
...
/**@brief Handle Glucose measurement client characteristic configuration descriptor write event.
 *
 * @param[in]   p_gls         Service instance.
 * @param[in]   p_evt_write   WRITE event to be handled.
 */
...
/**@brief TX_COMPLETE event handler.
 *
