...
/**@brief Make a HID Service characteristic id.
 *
...
static void on_connect(ble_hids_t * p_hids, ble_evt_t * p_ble_evt)
{
    ...
    err_code = ble_gatts_value_set(p_hids->protocol_mode_handles.value_handle, 
                                   0, 
                                   &len,
...
/**@brief Handle write events to the HID Control Point value.
 *
 * @param[in]   p_hids        HID Service structure.
...
/**@brief Handle write events to the Protocol Mode value.
 *
 * @param[in]   p_hids        HID Service structure.
 * @param[in]   p_evt_write   Write event received from the BLE stack.
 */
...
/**@brief Handle write events to a report CCCD.
 *
 * @param[in]   p_hids        HID Service structure.
...
/**@brief Handle write events to a report value.
 *
 * @param[in]   p_hids      HID Service structure.
 * @param[in]   p_char_id   Id of report characteristic.
 */
static void on_report_value_write(ble_hids_t * p_hids, ble_hids_char_id_t * p_char_id)
{
    if (p_hids->evt_handler != NULL)
    {
        ble_hids_evt_t evt;
...
/**@brief Find Characteristic Id of characteristic corresponding to a CCCD handle.
 *
 * @param[in]   p_hids      HID Service structure.
...
/**@brief Find Characteristic Id of characteristic corresponding to a value handle.
 *
 * @param[in]   p_hids      HID Service structure.
 * @param[in]   handle      Handle to search for.
 * @param[out]  p_char_id   Id of report characteristic.
...
static void on_write(ble_hids_t * p_hids, ble_evt_t * p_ble_evt)
{
    ...
    {
        char_id = make_char_id(BLE_UUID_BOOT_KEYBOARD_INPUT_REPORT_CHAR, 0, 0);
        on_report_value_write(p_hids, &char_id);
    }
...
/**@brief Add Protocol Mode characteristic.
 *
 * @param[in]   p_hids       HID Service structure.
                                       ...
                                       const ble_srv_security_mode_t * p_sec_mode)
{
    ...
    char_md.char_properties.write_wo_resp = 1;
    ...
    return ble_gatts_characteristic_add(p_hids->service_handle,
                                        &char_md,
                                        &attr_char_value,
...
/**@brief Add report characteristic.
 *
 * @param[in]   p_hids              HID Service structure.
...
static uint32_t rep_char_add(ble_hids_t *                   p_hids,
                             ble_gatts_char_properties_t *  p_properties,
                             uint8_t                        max_len,
                             ...
                             ble_hids_rep_char_t *          p_rep_char)
{
    ...
    char_md.char_properties  = *p_properties;
    char_md.p_char_user_desc = NULL;
    char_md.p_char_pf        = NULL;
    char_md.p_user_desc_md   = NULL;
    ...
    err_code = ble_gatts_characteristic_add(p_hids->service_handle,
                                            &char_md,
                                            &attr_char_value,
    ...
    return ble_gatts_descriptor_add(p_rep_char->char_handles.value_handle,
                                    &attr_char_value,
                                    &p_rep_char->ref_handle);
}


/**@brief Add Report Map characteristic.
 *
 * @param[in]   p_hids        HID Service structure.
...
static uint32_t rep_map_char_add(ble_hids_t * p_hids, const ble_hids_init_t * p_hids_init)
{
    ...
    if (p_hids_init->rep_map.p_ext_rep_ref != NULL)
    {
        ...
        err_code = ble_uuid_encode(p_hids_init->rep_map.p_ext_rep_ref,
                                   &encoded_rep_ref_len,
                                   encoded_rep_ref);
        if (err_code != NRF_SUCCESS)
        {
        ...
        err_code = ble_gatts_descriptor_add(p_hids->rep_map_handles.value_handle,
                                            &attr_char_value,
...
/**@brief Add Input Report characteristic.
 *
 * @param[in]   p_hids           HID Service structure.
                                      ...
                                      ble_gatts_char_handles_t *           p_char_handles)
{
    ...
    char_md.char_properties.read   = 1;
    char_md.char_properties.write  = (p_sec_mode->write_perm.sm) ? 1 : 0;
...
/**@brief Add Boot Keyboard Output Report characteristic.
 *
 * @param[in]   p_hids        HID Service structure.
 * @param[in]   p_hids_init   Service initialization structure.
 *
...
static uint32_t boot_kb_outp_rep_char_add(ble_hids_t * p_hids, const ble_hids_init_t * p_hids_init)
{
    ...
    char_md.char_properties.read          = 1;
    char_md.char_properties.write         = 1;
    char_md.char_properties.write_wo_resp = 1;
    char_md.p_char_user_desc              = NULL;
    char_md.p_char_pf                     = NULL;
    char_md.p_user_desc_md                = NULL;
    ...
    return ble_gatts_characteristic_add(p_hids->service_handle, &char_md, &attr_char_value,
                                        &p_hids->boot_kb_outp_rep_handles);
}


/**@brief Encode a HID Information characteristic value.
 *
                                      ...
                                      const ble_hids_hid_information_t * p_hid_information)
{
    ...
    ASSERT(len == ENCODED_HID_INFORMATION_LEN);
    return len;
}


/**@brief Add HID Information characteristic.
 *
 * @param[in]   p_hids        HID Service structure.
 * @param[in]   p_hids_init   Service initialization structure.
 *
...
static uint32_t hid_information_char_add(ble_hids_t * p_hids, const ble_hids_init_t * p_hids_init)
{
    ...
    return ble_gatts_characteristic_add(p_hids->service_handle, &char_md, 
                                        &attr_char_value, 
                                        &p_hids->hid_information_handles);
}


/**@brief Add HID Control Point characteristic.
 *
 * @param[in]   p_hids       HID Service structure.
 * @param[in]   p_sec_mode   Characteristic security settings.
 *
...
/**@brief Add input report characteristics.
 *
 * @param[in]   p_hids        HID Service structure.
 * @param[in]   p_hids_init   Service initialization structure.
 *
                                            ...
                                            const ble_hids_init_t * p_hids_init)
{
    if ((p_hids_init->inp_rep_count != 0) && (p_hids_init->p_inp_rep_array != NULL))
    {
        ...
        for (i = 0; i < p_hids_init->inp_rep_count; i++)
        {
            ...
            ble_gatts_char_properties_t properties;
...
/**@brief Add output report characteristics.
 *
 * @param[in]   p_hids        HID Service structure.
 * @param[in]   p_hids_init   Service initialization structure.
 *
...
/**@brief Add feature report characteristics.
 *
 * @param[in]   p_hids        HID Service structure.
 * @param[in]   p_hids_init   Service initialization structure.
 *
...
static uint32_t includes_add(ble_hids_t * p_hids, const ble_hids_init_t * p_hids_init)
{
    ...
    {
        err_code = ble_gatts_include_add(p_hids->service_handle,
...
uint32_t ble_hids_init(ble_hids_t * p_hids, const ble_hids_init_t * p_hids_init)
{
    ...
    if (
        (p_hids_init->inp_rep_count > BLE_HIDS_MAX_INPUT_REP)
        ||
    ...
    err_code = ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY,
                                     &ble_uuid,
                               ...
                               uint8_t *    p_data)
{
    ...
    if (rep_index < p_hids->inp_rep_count)
    {
        ...
        if (p_hids->conn_handle != BLE_CONN_HANDLE_INVALID)
        {
            ...
            err_code = ble_gatts_hvx(p_hids->conn_handle, &hvx_params);
                                          ...
                                          uint8_t *    p_optional_data)
{
    ...
    if (p_hids->conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        if (hvx_len <= BOOT_MOUSE_INPUT_REPORT_MAX_SIZE)
        {
            ...
            ASSERT(BOOT_MOUSE_INPUT_REPORT_MIN_SIZE == 3);
...
uint32_t ble_hids_outp_rep_get(ble_hids_t * p_hids,
                               ...
                               uint8_t *    p_outp_rep)
{
    return ble_gatts_value_get(p_hids->outp_rep_array[rep_index].char_handles.value_handle,
                               offset,
                               &len,
                               p_outp_rep);
}
