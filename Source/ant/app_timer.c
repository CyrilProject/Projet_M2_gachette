...
static uint32_t rtc_init(void)
{
    ...
    err_code = nrf_nvic_ClearPendingIRQ(RTC1_IRQn);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    err_code = nrf_nvic_SetPriority(RTC1_IRQn, NRF_APP_PRIORITY_LOW);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    err_code = nrf_nvic_EnableIRQ(RTC1_IRQn);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
