...
/**@brief Handle write events to the Heart Rate Measurement characteristic.
 *
 * @param[in]   p_hrs         Heart Rate Service structure.
...
/**@brief Add Heart Rate Measurement characteristic.
 *
 * @param[in]   p_hrs        Heart Rate Service structure.
                                                ...
                                                const ble_hrs_init_t * p_hrs_init)
{
    ...
    return ble_gatts_characteristic_add(p_hrs->service_handle,
                                        &char_md,
                                        &attr_char_value,
                                        &p_hrs->hrm_handles);
}
...
/**@brief Add Body Sensor Location characteristic.
 *
 * @param[in]   p_hrs        Heart Rate Service structure.
 * @param[in]   p_hrs_init   Information needed to initialize the service.
 *
...
uint32_t ble_hrs_init(ble_hrs_t * p_hrs, const ble_hrs_init_t * p_hrs_init)
{
    ...
    err_code = ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &p_hrs->service_handle);
...
uint32_t ble_hrs_heart_rate_measurement_send(ble_hrs_t * p_hrs, uint16_t heart_rate)
{
    ...
    if (p_hrs->conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        err_code = ble_gatts_hvx(p_hrs->conn_handle, &hvx_params);
...
uint32_t ble_hrs_body_sensor_location_set(ble_hrs_t * p_hrs, uint8_t body_sensor_location)
{
    ...
    return ble_gatts_value_set(p_hrs->bsl_handles.value_handle, 0, &len, &body_sensor_location);
}
