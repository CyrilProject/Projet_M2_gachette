...
#define CHANNEL_TYPE SLAVE_RX_ONLY                 ((uint8_t) 0x40) ///< Slave rx only channel (PARAMETER_RX_NOT_TX | PARAMETER_RX_ONLY).
...
#define CAPABILITIES_SEARCH_SCAN_MODE_ENABLED      ((uint8_t)0x08) ///< Bitfield in CAPABILITIES_ADVANCED_3 byte indicating search scan mode support.
#define CAPABILITIES_SEARCH_SHARING_MODE_ENABLED   ((uint8_t)0x10) ///< Bitfield in CAPABILITIES_ADVANCED_3 byte indicating search sharing mode support.
