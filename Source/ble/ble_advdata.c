                            ...
                            uint8_t *             p_len)
{
    ...
    // Get GAP device name
    err_code = ble_gap_device_name_get(&p_encoded_data[*p_len + 2], &name_length);
...
static uint32_t appearance_encode(uint8_t * p_encoded_data, uint8_t * p_len)
{
    ...
    err_code = ble_gap_appearance_get(&appearance);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
                                       ...
                                       uint8_t *                       p_len)
{
    ...
    for (i = 0; i < p_uuid_list->uuid_cnt; i++)
    {
        ...
        // Find size
        err_code = ble_uuid_encode(&uuid, &encoded_size, NULL);
        if (err_code != NRF_SUCCESS)
        {
            return err_code;
        }
        ...
        if (encoded_size == uuid_size)
        {
            ...
            // Write UUID
            err_code = ble_uuid_encode(&uuid, &encoded_size, &p_encoded_data[*p_len]);
            if (err_code != NRF_SUCCESS)
            {
                return err_code;
            }
...
static uint32_t conn_int_check(const ble_advdata_conn_int_t *p_conn_int)
{
    ...
    if (
        (p_conn_int->min_conn_interval < 0x0006)
        ||
        (
    ...
    if (
        (p_conn_int->max_conn_interval < 0x0006)
        ||
        (
...
uint32_t ble_advdata_set(const ble_advdata_t * p_advdata, const ble_advdata_t * p_srdata)
{
    ...
    return ble_gap_adv_data_set(p_encoded_advdata, len_advdata, p_encoded_srdata, len_srdata);
}
