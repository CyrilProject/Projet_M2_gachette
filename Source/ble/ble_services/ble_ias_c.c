...
static uint32_t on_connect(ble_ias_c_t * p_ias_c, ble_evt_t const * p_ble_evt)
{
    ...
    return ble_gattc_primary_services_discover(p_ias_c->conn_handle, START_HANDLE, &ias_uuid);
}


/**@brief Primary Service Discovery Response event handler.
 *
 * @param[in]   p_ias_c     Immediate Alert Service client structure.
 * @param[in]   p_ble_evt   Event received from the BLE stack.
 */
static uint32_t on_srv_disc_resp(ble_ias_c_t * p_ias_c, ble_evt_t const * p_ble_evt)
{
    ...
    else
    {
        ...
        err_code = ble_gattc_characteristics_discover(p_ias_c->conn_handle, p_service_handle_range);
    }
...
/**@brief Characteristic Discovery Response event handler.
 *
 * @param[in]   p_ias_c     Immediate Alert Service client structure.
 * @param[in]   p_ble_evt   Event received from the BLE stack.
 */
    ...
    if (p_ble_evt->evt.gattc_evt.gatt_status == BLE_GATT_STATUS_SUCCESS)
    {
        ...
        for (i = 0; i < p_char_disc_rsp->count; i++)
        {
            if (BLE_UUID_EQ(&m_alert_level_uuid, &(p_char_disc_rsp->chars[i].uuid)))
            {
                ...
                if (p_char_disc_rsp->chars[i].properties & 0x04)
                {
...
/**@brief Perform a Write procedure.
 *
                                           ...
                                           uint8_t * p_value)
{
    ...
    return ble_gattc_write(conn_handle, &write_params);
}
