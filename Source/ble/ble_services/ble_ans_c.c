...
typedef struct
{
    ...
    uint8_t                  properties;                                /**< Properties for this characteristic. */
...
static void process_tx_buffer(void)
{
    if (m_tx_index != m_tx_insert_index)
    {
        ...
        {
            err_code = ble_gattc_read(m_tx_buffer[m_tx_index].conn_handle,
                                      m_tx_buffer[m_tx_index].req.read_handle,
                                      0);
        }
        else
        {
            err_code = ble_gattc_write(m_tx_buffer[m_tx_index].conn_handle,
...
static void action_service_req(const ble_ans_c_t * p_ans)
{
    ...
    err_code = ble_gattc_primary_services_discover(p_ans->conn_handle, handle, &ans_uuid);
    if (err_code != NRF_SUCCESS)
    {
                                      ...
                                      const ble_gattc_handle_range_t * p_handle)
{
    ...
    err_code = ble_gattc_characteristics_discover(p_ans->conn_handle, p_handle);
...
static void action_descriptor_req(const ble_ans_c_t * p_ans)
{
    ...
    if (m_service.new_alert.handle_cccd == BLE_ANS_INVALID_HANDLE)
    {
        ...
        err_code = ble_gattc_descriptors_discover(p_ans->conn_handle, &descriptor_handle);
    }
                                ...
                                const ble_gattc_char_t * p_char_resp)
{
    ...
    p_characteristic->properties   = p_char_resp->properties;
...
void ble_ans_c_on_ble_evt(ble_ans_c_t * p_ans, const ble_evt_t * p_ble_evt)
{
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GATTC_EVT_MTU_XCHG_RSP:
