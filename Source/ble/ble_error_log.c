...
uint32_t ble_error_log_write(uint32_t err_code, const uint8_t * p_message, uint16_t line_number)
{
    ...
    strcpy((char *)m_ble_error_log.message, (const char *)p_message);
...
uint32_t ble_error_log_read(ble_error_log_data_t * error_log)
{
    ...
    m_ble_error_log.failure       = 0;
    m_ble_error_log.line_number   = 0;
    m_ble_error_log.message[0]    = '\0';
    m_ble_error_log.stack_info[0] = 0;

    err_code = ble_flash_page_write(FLASH_PAGE_ERROR_LOG,
                                    (uint32_t *) &m_ble_error_log,
                                    error_log_size);
