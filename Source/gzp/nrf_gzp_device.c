...
//static uint32_t latest_tx_pipe;
...
void gzp_delay_rx_periods(uint16_t timeslot_periods)
{
    uint16_t i;

    for(i=0; i < timeslot_periods; i++)
    {
        nrf_delay_us(nrf_gzll_get_timeslot_period()*2);
    }
}
