...
static uint16_t            m_crc_bond;                                    /**< Combined CRC for all bonding information currently stored in flash. */
static uint32_t *          mp_flash_bond;                                 /**< Pointer to flash location to write next bonding information. */
static uint8_t             m_master_bonds_in_flash_count;                 /**< Number of bonded masters currently stored in flash. */
...
/**@brief Store specified master to flash.
 *
 * @param[in]  p_bond   Bonding information to be stored.
 *
 * @return     NRF_SUCCESS on success, an error_code otherwise.
 */
static uint32_t master_bond_store(master_bond_t * p_bond)
{
    uint32_t err_code;

    // Check if flash is full
    if (m_master_bonds_in_flash_count >= MAX_BONDS_IN_FLASH)
    {
        return NRF_ERROR_NO_MEM;
    }

    // Check if this is the first bond to be stored
    if (m_master_bonds_in_flash_count == 0)
    {
        // Initialize CRC
        m_crc_bond = ble_flash_crc16_compute(NULL, 0, NULL);
    
        // Find pointer to start of bond information flash block
        err_code = ble_flash_page_addr(m_bondmngr_config.flash_page_num_bond, &mp_flash_bond);
    ...
    err_code = ble_flash_block_write(mp_flash_bond + 1,
                                     (uint32_t *)p_bond,
    ...
    m_crc_bond = ble_flash_crc16_compute((uint8_t *)p_bond, sizeof(master_bond_t), &m_crc_bond);
    
    // Write header
    err_code = ble_flash_word_write(mp_flash_bond, BLE_FLASH_MAGIC_NUMBER | m_crc_bond);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    ...
    mp_flash_bond += (sizeof(master_bond_t) / sizeof(uint32_t)) + 1;
    
    m_master_bonds_in_flash_count++;
    return NRF_SUCCESS;
}
 ...
 */
static uint32_t master_bond_load(master_bond_t * p_bond)
{
    uint32_t err_code;
    uint16_t crc_header;

    // Check if this is the first bond to be loaded
    if (m_master_bonds_in_flash_count == 0)
    {
        // Initialize CRC
        m_crc_bond = ble_flash_crc16_compute(NULL, 0, NULL);
    ...
    err_code = crc_extract(*mp_flash_bond, &crc_header);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    ...
    // Load master
    *p_bond = *(master_bond_t *)(mp_flash_bond + 1);
    
    // Check CRC
    m_crc_bond = ble_flash_crc16_compute((uint8_t *)p_bond, sizeof(master_bond_t), &m_crc_bond);
    if (m_crc_bond == crc_header)
    {
        m_master_bonds_in_flash_count++;
        mp_flash_bond += (sizeof(master_bond_t) / sizeof(uint32_t)) + 1;
...
/**@brief Store system attribute information for all masters in flash.
 *
 * @param[in]  crc   Precomputed CRC value for system attribute flash block.
 *
 * @return     NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t sys_attr_store(uint16_t crc)
{
    uint32_t   err_code;
    uint32_t * p_page;
    uint32_t * p_flash;
    ...
    // Find pointer to start of system attribute information flash block
    err_code = ble_flash_page_addr(m_bondmngr_config.flash_page_num_sys_attr, &p_page);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    
    // Store number of system attribute records
    err_code = ble_flash_word_write(p_page + 1, m_masters_in_db_count);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    ...
    {
        master_sys_attr_t * p_sys_attr = &m_masters_db[i].sys_attr;
        
        err_code = ble_flash_block_write(p_flash,
                                         (uint32_t *)p_sys_attr,
        ...
        p_flash += sizeof(master_sys_attr_t) / sizeof(uint32_t);
    }
    
    // Store header
    return ble_flash_word_write(p_page, BLE_FLASH_MAGIC_NUMBER | crc);
}


/**@brief Load system attribute information for all masters from flash.
 *
 * @return     NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t sys_attr_load(void)
{
    uint32_t   err_code;
    uint16_t   crc_header;
    uint16_t   crc;
    ...
    // Extract CRC from header
    err_code = crc_extract(*p_page, &crc_header);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    
    // Load number of system attribute records
    master_count = *(p_page + 1);
    if (master_count > BLE_BONDMNGR_MAX_BONDED_MASTERS)
    {
        return NRF_ERROR_INVALID_DATA;
    }
    
    // Initialize CRC
    crc = ble_flash_crc16_compute((uint8_t *)&master_count, sizeof(master_count), NULL);
    
    // Load system attributes for all masters
    p_flash = p_page + 2;
    for (i = 0; i < master_count; i++)
    {
        master_sys_attr_t * p_sys_attr = &m_masters_db[i].sys_attr;
        
        *p_sys_attr = *(master_sys_attr_t *)p_flash;
        crc = ble_flash_crc16_compute((uint8_t *)p_sys_attr, sizeof(master_sys_attr_t), &crc);
        p_flash += sizeof(master_sys_attr_t) / sizeof(uint32_t);
    }
    
    // Check CRC
    return (crc == crc_header) ? NRF_SUCCESS : NRF_ERROR_INVALID_DATA;
}
 ...
 */
static bool master_bonds_changed(void)
{
    int      i;
    ...
    return (crc != m_crc_bond);
}


/**@brief Check if system attribute information in memory is different from the one in flash.
 *
 * @param[out] p_changed   TRUE if system attribute information has changed, FALSE otherwise.
 * @param[out] p_crc_new   CRC value for system attribute information in memory.
 *
 * @return     NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t sys_attr_changed(bool * p_changed, uint16_t * p_crc_new)
{
    uint32_t   err_code;
    uint32_t * p_page;
    uint16_t   crc_flash;
    int        i;
    uint32_t   master_count = m_masters_in_db_count;
    
    // Compute CRC of system attributes in database
    *p_crc_new = ble_flash_crc16_compute((uint8_t *)&master_count, sizeof(master_count), NULL);
    
    for (i = 0; i < m_masters_in_db_count; i++)
    {
        *p_crc_new = ble_flash_crc16_compute((uint8_t *)&m_masters_db[i].sys_attr,
    ...
    // Extract CRC from header
    err_code = crc_extract(*p_page, &crc_flash);
    switch (err_code)
    {
        case NRF_SUCCESS:
            // Compare to CRC of system attributes in flash
            *p_changed = (*p_crc_new != crc_flash);
            break;
            
        case NRF_ERROR_NOT_FOUND:
            // No system attributes in flash, update flash if there are masters in database
            *p_changed = (m_masters_in_db_count > 0);
            break;
            
        case NRF_ERROR_INVALID_DATA:
            // Content of flash is invalid, always update flash
            *p_changed = true;
            break;
...
static uint32_t master_sys_attr_set(master_t * p_master)
{
    ...
    return ble_gatts_sys_attr_set(m_conn_handle, p_sys_attr, m_master.sys_attr.sys_attr_size);
}
...
/**@brief Add master to database.
 *
 * @details Also stores the master's bonding information to flash, and notifies the application that
 ...
 * @return     NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t master_add(ble_gap_evt_auth_status_t * p_auth_status)
{
    uint32_t err_code;
    ...
    err_code = ble_gatts_sys_attr_set(m_conn_handle, NULL, 0);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    ...
    err_code = master_bond_store(&m_master.bond);
 ...
 *
 * @param[in]  diversificator   Diversificator to search for.
 */
static uint32_t master_find(uint16_t master_div)
{
    int i;
 ...
 * @return     NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t bondmngr_load(void)
{
    uint32_t err_code;
    int      i;
    ...
    {
        master_bond_t master_bond;
        
        // Load bonding information
        err_code = master_bond_load(&master_bond);
        ...
        if (master_bond.master_handle > m_masters_in_db_count)
        {
        ...
        else
        {
            ...
            m_masters_db[master_bond.master_handle].bond = master_bond;
            if (master_bond.master_handle == m_masters_in_db_count)
            {
                // New master handle, clear system attribute information
                m_masters_db[master_bond.master_handle].sys_attr.sys_attr_size = 0;
...
/**@brief Authentication Status Updated event handler.
 *
 * @param[in]  p_ble_evt   Event received from the BLE stack.
 */
static void on_auth_status(ble_gap_evt_auth_status_t * p_auth_status)
{
    ...
    if (m_master.bond.master_handle == INVALID_MASTER_HANDLE)
    {
        uint32_t err_code = master_find(p_auth_status->periph_keys.enc_info.div);
        
        switch (err_code)
        {
            case NRF_SUCCESS:
                // Master found in the list of bonded master, set the corresponding sys_attr
                err_code = master_sys_attr_set(&m_master);
                break;
                
            case NRF_ERROR_NOT_FOUND:
                // Master not found, add new master
                err_code = master_add(p_auth_status);
                break;
...
static void on_sec_info_request(ble_evt_t * p_ble_evt)
{
    ...
    err_code = master_find(m_master.bond.master_id_info.div);
    if (err_code == NRF_SUCCESS)
    {
        // Master found in the list of bonded master. Use the encryption info for this master.
        err_code = ble_gap_sec_info_reply(m_conn_handle, 
    ...
    {
        // New master
        err_code = ble_gap_sec_info_reply(m_conn_handle, NULL, NULL);
        if (err_code != NRF_SUCCESS)
        {
            m_bondmngr_config.error_handler(err_code);
        }
...
void ble_bondmngr_on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        //       Disconnected handler of the application before advertising is started (to make
...
uint32_t ble_bondmngr_bonded_masters_store(void)
{
    ...
    if (m_master.bond.master_handle != INVALID_MASTER_HANDLE)
    {
        ...
        err_code = ble_gatts_sys_attr_get(m_conn_handle,
    ...
    if (master_bonds_changed())
    {
        ...
        m_master_bonds_in_flash_count = 0;
        for (i = 0; i < m_masters_in_db_count; i++)
        {
            err_code = master_bond_store(&m_masters_db[i].bond);
            if (err_code != NRF_SUCCESS)
            {
                return err_code;
            }
    ...
    err_code = sys_attr_changed(&changed, &crc_sys_attr_new);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
    if (changed)
    {
        ...
        err_code = sys_attr_store(crc_sys_attr_new);
        if (err_code != NRF_SUCCESS)
        {
            return err_code;
        }
