...
static uint32_t alert_level_char_add(ble_lls_t * p_lls, const ble_lls_init_t * p_lls_init)
{
    ...
    return ble_gatts_characteristic_add(p_lls->service_handle,
                                        &char_md,
                                        &attr_char_value,
...
uint32_t ble_lls_init(ble_lls_t * p_lls, const ble_lls_init_t * p_lls_init)
{
    ...
    err_code = ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &p_lls->service_handle);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_lls_alert_level_get(ble_lls_t * p_lls, uint8_t * p_alert_level)
{
    ...
    return ble_gatts_value_get(p_lls->alert_level_handles.value_handle, 0, &len, p_alert_level);
}
