...
bool cherry8x16_new_packet(const uint8_t ** p_key_packet, uint8_t *p_key_packet_size)
{
  ...
  {
    if (cherry8x16_have_keys_changed(m_currently_pressed_keys, m_num_of_currently_pressed_keys, m_transmitted_keys, m_number_of_transmitted_keys))
    {
...
/**
 * Remaps keypad, F11 and F12 keys in case when Fn key is pressed.
 *
...
/**
 * Determines whether keyboard matrix state has changed compared to state before.
 *
