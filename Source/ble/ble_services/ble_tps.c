...
/**@brief Add TX Power Level characteristic.
 *
                                        ...
                                        const ble_tps_init_t * p_tps_init)
{
    ...
    return ble_gatts_characteristic_add(p_tps->service_handle,
                                        &char_md,
                                        &attr_char_value,
...
uint32_t ble_tps_init(ble_tps_t * p_tps, const ble_tps_init_t * p_tps_init)
{
    ...
    err_code = ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &p_tps->service_handle);
...
uint32_t ble_tps_tx_power_level_set(ble_tps_t * p_tps, int8_t tx_power_level)
{
    ...
    // Update database
    return ble_gatts_value_set(p_tps->tx_power_level_handles.value_handle, 
                               0, 
                               &len, 
