...
{
    ASSERT(p_sys_id != NULL);
    ASSERT(p_encoded_buffer != NULL);
                         ...
                         ble_gatts_char_handles_t *      p_handles)
{
    ...
    return ble_gatts_characteristic_add(service_handle, &char_md, &attr_char_value, p_handles);
}
...
uint32_t ble_dis_init(const ble_dis_init_t * p_dis_init)
{
    ...
    err_code = ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &service_handle);
