...
/**@brief Encode a Scan Interval Window.
 *
...
/**@brief Add Scan Interval Window characteristic.
 *
 * @param[in]   p_sps        Scan Parameters Service structure.
...
static uint32_t scan_int_window_char_add(ble_sps_t * p_sps, const ble_sps_init_t * p_sps_init)
{
    ...
    return ble_gatts_characteristic_add(p_sps->service_handle, 
                                        &char_md,
                                        &attr_char_value,
...
/**@brief Add Scan Refresh characteristic.
 *
 * @param[in]   p_sps        Scan Parameters Service structure.
 * @param[in]   p_sps_init   Information needed to initialize the service.
 *
...
static uint32_t scan_refresh_char_add(ble_sps_t * p_sps, const ble_sps_init_t * p_sps_init)
{
    ...
    err_code = ble_gatts_characteristic_add(p_sps->service_handle,
                                            &char_md,
                                            &attr_char_value,
...
uint32_t ble_sps_init(ble_sps_t * p_sps, const ble_sps_init_t * p_sps_init)
{
    ...
    err_code = ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &p_sps->service_handle);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_bas_battery_level_update(ble_sps_t * p_sps, uint8_t battery_level)
{
    ...
    if (battery_level != p_sps->battery_level_last)
    {
        ...
        // Update database
        err_code = ble_gatts_value_set(p_sps->battery_level_handles.value_handle,
                                       0,
                                       &len,
                                       &battery_level);
        if (err_code != NRF_SUCCESS)
        ...
        if ((p_sps->conn_handle != BLE_CONN_HANDLE_INVALID) && p_sps->is_notifying)
        {
            ...
            err_code = ble_gatts_hvx(p_sps->conn_handle, &hvx_params);
        }
        else
        {
