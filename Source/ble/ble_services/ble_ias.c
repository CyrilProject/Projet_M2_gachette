...
/**@brief Add Alert Level characteristic.
 *
 * @param[in]   p_ias        Immediate Alert Service structure.
...
static uint32_t alert_level_char_add(ble_ias_t * p_ias)
{
    ...
    return ble_gatts_characteristic_add(p_ias->service_handle,
                                        &char_md,
                                        &attr_char_value,
...
uint32_t ble_ias_init(ble_ias_t * p_ias, const ble_ias_init_t * p_ias_init)
{
    ...
    err_code = ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &p_ias->service_handle);
...
uint32_t ble_ias_alert_level_get(ble_ias_t * p_ias, uint8_t * p_alert_level)
{
    ...
    return ble_gatts_value_get(p_ias->alert_level_handles.value_handle, 0, &len, p_alert_level);
}
